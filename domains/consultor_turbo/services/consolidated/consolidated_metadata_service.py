...
class ConsultorTurboConsolidatedMetadataService:
    ...
    def _check_column_in_s3(self, name_temp: str, s3_file_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if s3_file_data:
            for s3_col in s3_file_data.get('columns', []):
                if self._normalize_name(s3_col.get('name', '')) == self._normalize_name(name_temp):
                    s3_column_info = {
                        'exists_in_s3': True,
                        's3_name': s3_col.get('name', '')
                    }
                    break
            if not s3_column_info:
    ...
    def consolidate(self) -> None:
        ...
        s3_comparison_path = self.base_dir / "output" / "consultor_turbo" / "s3_vs_etl" / "s3_vs_etl_metadata.json"
        system_comparison_path = self.base_dir / "output" / "consultor_turbo" / "etl_vs_system" / "etl_vs_system_metadata.json"
        ...
        for file_data in s3_metadata.get('files', []):
            ...
            s3_files_map[base_name] = file_data
        ...
        for etl_config in etl_metadata.get('configs', []):
            ...
            for col in etl_config.get('columns', []):
                ...
                s3_column_info = self._check_column_in_s3(name_temp, s3_file_data)
                
                # Get is_used_in_system from comparison data (true = usado, false = não usado)
                system_col_data = system_column_map.get(name_file, {})
                ...
                if is_unused:
                    columns_not_in_system.append({
                        ...
                        'type_santander': col.get('TypeTemp', ''),
                        'from_etl': name_file,
                        'type_etl': col.get('TypeFile', ''),
            ...
            table_info = {
                ...
                'temp_table': etl_config.get('temp_table', ''),
                'temp_db': etl_config.get('temp_db', ''),
                'version': etl_config.get('version', 'N/A'),
                'keys': etl_config.get('keys', []),
                'update_fields': etl_config.get('update_fields', []),
                'period_column': etl_config.get('period_column', ''),
                'soft_delete': {
                ...
                's3_comparison': {
                    ...
                    's3_file_name': s3_file_data.get('file_name', '') if s3_file_data else '',
