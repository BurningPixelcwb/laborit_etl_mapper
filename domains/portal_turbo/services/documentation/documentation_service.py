...
class PortalTurboDocumentationService:
    ...
    def _generate_file_markdown(self, table_info: Dict[str, Any], s3_comparison: Dict[str, Any] = None) -> str:
        ...
        # Extract module prefix from output_path (e.g., "portal_turbo" -> "portal")
        # output_path is like: output/portal_turbo/ or output/consultor_turbo/
        ...
        # Get S3 comparison data from table_info (consolidated metadata) or from s3_comparison parameter
        s3_comp_data = table_info.get('s3_comparison', {}) or s3_comparison or {}
        
        # Add columns only in S3 section (after main table)
        ...
        # Add columns only in ETL section (after S3 table)
    ...
    def generate_documentation(self):
        ...
        for table_info in tables:
            try:
                ...
                # Get S3 comparison data for this table (fallback to s3_comparison_map if not in table_info)
                s3_comparison = None
                if not table_info.get('s3_comparison', {}).get('columns_only_in_s3') and not table_info.get('s3_comparison', {}).get('columns_only_in_etl'):
                    # Fallback to s3_comparison_map if data not in table_info
