"""
Service to compare S3 vs ETL metadata for Portal Turbo
"""
...
class PortalTurboS3ComparatorService:
    ...
    def compare_metadatas(self, s3_metadata: Dict[str, Any], etl_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Create mapping by mapa name (from S3) and config_name (from ETL)
        s3_files = {}
        for file_data in s3_metadata.get('files', []):
            mapa_name = file_data.get('mapa', '')
            if mapa_name:
                s3_files[mapa_name] = file_data
        
        etl_configs = {config['config_name']: config for config in etl_metadata.get('configs', [])}
        
        # Compare each S3 file with ETL config
        for mapa_name, s3_file_data in s3_files.items():
            # Try to find ETL config by config_name matching mapa_name
            etl_config = etl_configs.get(mapa_name, {})
            
            # If not found, try to find by final_table matching s3 table
            if not etl_config:
                s3_table = s3_file_data.get('table', '')
                for config_name, config in etl_configs.items():
                    if config.get('final_table') == s3_table:
                        etl_config = config
                        break
            
            s3_columns = {col['normalized_name']: col for col in s3_file_data.get('columns', [])}
            etl_columns = {}
            
            if etl_config:
                # Extract columns from ETL config - compare using NameTemp (std name)
            ...
            for s3_norm, s3_col_data in s3_columns.items():
                s3_col_name = s3_col_data.get('name', '')
                if s3_norm in etl_columns:
                    columns_in_both.append({
                        's3_name': s3_col_name,
                        's3_laborit': s3_col_data.get('laborit_name', ''),
                        's3_type': s3_col_data.get('type', ''),
                    ...
                    columns_only_in_s3.append(s3_col_name)
            ...
            print(f"   {status} {mapa_name}: S3={s3_file_data.get('columns_count', 0)} | ETL={len(etl_config.get('columns', [])) if etl_config else 0} | Both={len(columns_in_both)} | Only S3={len(columns_only_in_s3)} | Only ETL={len(columns_only_in_etl)}")
    ...
    def compare(self):
        ...
        print("🚀 Starting S3 vs ETL metadata comparison - Portal Turbo...")
