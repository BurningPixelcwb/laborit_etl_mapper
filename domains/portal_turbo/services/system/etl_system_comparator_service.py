"""
Service to compare ETL vs System metadata for Portal Turbo
"""
...
class PortalTurboETLSystemComparatorService:
    ...
    def __init__(self, output_path: Path, etl_metadata_path: Path, system_metadata_path: Path):
        ...
        Args:
            ...
            system_metadata_path: Path to System metadata JSON (portal_system.json)
        """
        self.output_path = Path(output_path)
        self.etl_metadata_path = Path(etl_metadata_path)
        self.system_metadata_path = Path(system_metadata_path)
        # Calculate base_dir from output_path (output/portal_turbo -> project root)
        base_dir = self.output_path.parent.parent
        self.from_system_service = PortalTurboFromSystemService(
            base_dir=base_dir,
    ...
    def compare_metadatas(self, etl_metadata: Dict[str, Any], system_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Load all system metadata and consolidate by table name
        all_system_metadata = []
        if isinstance(system_metadata, list):
            all_system_metadata = system_metadata
        elif isinstance(system_metadata, dict):
            all_system_metadata = [system_metadata]
        
        # Create mapping by table name, consolidating all mapas for the same table
        system_map = {}
        for item in all_system_metadata:
            tabela = item.get('tabela', '')
            if tabela:
                if tabela not in system_map:
                    system_map[tabela] = {
                        'tabela': tabela,
                        'mapas': [],
                ...
                # Add mapa to list
                system_map[tabela]['mapas'].append(item.get('mapa', ''))
                # Consolidate unused columns (avoid duplicates)
                existing_cols = set(system_map[tabela]['colunas_nao_utilizadas'])
                for col in item.get('colunas_nao_utilizadas', []):
                    if col not in existing_cols:
                        system_map[tabela]['colunas_nao_utilizadas'].append(col)
        
        comparison['system_metadata']['total_tables'] = len(system_map)
        ...
        for config_name, etl_config in etl_configs.items():
            ...
            for col in etl_config.get('columns', []):
                name_file = col.get('NameFile', '') or ''
                name_temp = col.get('NameTemp', '') or ''
                
                normalized_name_file = self.normalize_column_name(name_file)
                is_unused = normalized_name_file in unused_columns_set if normalized_name_file else False
            ...
            comparison_item = {
                ...
                'system_mapas': system_data.get('mapas', []) if system_data else [],
    ...
    def compare(self):
        ...
        print("🚀 Starting ETL vs System metadata comparison - Portal Turbo...")
