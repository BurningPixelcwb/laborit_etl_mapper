"""
Service to publish Consultor Turbo consolidated metadata JSON directly to Confluence as Wiki Markup
"""
...
class ConsultorTurboJsonPublicationService:
    """Service to publish Consultor Turbo consolidated metadata JSON directly to Confluence as Wiki Markup"""
    
    def __init__(self, client: ConfluenceClient):
        """
        Initialize Consultor Turbo JSON publication service
        
        Args:
            client: ConfluenceClient instance
        """
        self.client = client
        self.project_name = 'consultor_turbo'
        # Extract first name from project_name (before underscore)
        # Example: "consultor_turbo" -> "consultor"
    ...
    def _json_to_wiki_table_main(self, columns: List[Dict[str, Any]], has_s3_data: bool = True) -> str:
        ...
        for column in columns:
            ...
            # Invert is_unused to used_in_sys
    ...
    def publish_from_json(
        ...
        """
        Publish Consultor Turbo consolidated metadata JSON to Confluence
        
        Args:
            json_path: Path to consolidated_metadata.json file.
                      If None, uses default path: output/consultor_turbo/consolidated_metadata/consolidated_metadata.json
            parent_page_title: Optional custom parent page title
            
        Returns:
            Dictionary with publication results
        """
        # Default path for consultor_turbo
        if json_path is None:
            base_path = Path(__file__).parent.parent.parent.parent.parent
            json_path = base_path / "output" / "consultor_turbo" / "consolidated_metadata" / "consolidated_metadata.json"
        else:
