#!/usr/bin/env python3
"""
Script para consolidar todos os metadados (ETL, S3, System) do Portal Turbo em um único JSON
"""
...
def main():
    """Consolida todos os metadados (ETL, S3, System) do Portal Turbo em um único JSON"""
    
    project_name = 'portal_turbo'
    
    print("=" * 70)
    print("🚀 Consolidando Metadados - Portal Turbo")
    print("=" * 70)
    ...
    try:
        ...
        etl_metadata_path = BASE_DIR / "output" / "portal_turbo" / "etl_metadata" / "portal_etl_metadata.json"
        s3_metadata_path = BASE_DIR / "output" / "portal_turbo" / "from_santander_metadata" / "from_santander_metadata.json"
        s3_comparison_path = BASE_DIR / "output" / "portal_turbo" / "s3_vs_etl" / "s3_vs_etl_metadata.json"
        system_comparison_path = BASE_DIR / "output" / "portal_turbo" / "etl_vs_system" / "etl_vs_system_metadata.json"
        ...
        if missing_files:
            ...
            print(f"   - python3 scripts/portal_turbo/portal_turbo_run_etl_metadata.py")
            print(f"   - python3 scripts/portal_turbo/portal_turbo_run_s3_metadata.py")
            print(f"   - python3 scripts/portal_turbo/portal_turbo_run_s3_etl_comparison.py")
            print(f"   - python3 scripts/portal_turbo/portal_turbo_run_etl_system_comparison.py")
            return 1
